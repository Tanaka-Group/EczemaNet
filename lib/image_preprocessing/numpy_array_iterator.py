...
class NumpyArrayIterator(Iterator):
    ...
    def __init__(self,
        ...
        if subset is not None:
            ...
            if subset == 'validation':
                ...
                if y is not None and type(y) is not dict:
                    y = y[:split_idx]
                if y is not None and type(y) is dict:
                    y_sub = {}
                    for key in y.keys():
                        y_sub[key] = y[key][:split_idx]
                    y = y_sub
            else:
                x = x[split_idx:]
                x_misc = [np.asarray(xx[split_idx:]) for xx in x_misc]
        ...
        if y is not None and type(y) is not dict:
            ...
            self.y = y
        if sample_weight is not None and type(y) is not dict:
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.y is None:
            return output[0]
        elif type(self.y) is dict:
            y_sub = {}
            for key in self.y.keys():
                y_sub[key] = self.y[key][index_array]
            output += (y_sub,)
        else:
