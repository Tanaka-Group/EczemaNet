...
# # Model Training: Baseline Study - Ensemble methods: Complex VS Simple
...
OUTPUT_PATH = "../output/Baseline_Simple_Ensemble/"
...
###########################################################################
# Overwritting the 'combined_statistics' function in EczemaNet_helper
###########################################################################
...
def predict(x_data, meta_data, model, branchlist=BRANCHES_LIST):
    ...
    for group_name, df_group in meta_grouped:
        ...
        for score_name in ALL_COMBINED_KEYS:
            y_whole_pred[score_name] = np.sum( [ y_whole_pred[key] for key in ALL_COMBINED_KEYS[score_name]], axis=0)
