...
# ## Model Training: EczemaNet Full-suite with Autoregressive Sign Dependency (ASD)
...
OUTPUT_PATH = "../output/EczemaNet_ASD/"
...
for run in range(0,10):
    ...
    # ------------------------------------------------
    # -- 4.1 Remove last layers --
    for branch in BRANCHES_LIST:
        pretrained_model.layers.pop()
        pretrained_model.layers.pop()
    # -- 4.2 Add new output layers --
    output_branches = []
    # 4.2.1 Erythema
    for layer in pretrained_model.layers:
        if "tiss_ery_dense512_2" in layer.name: # Add Original 512 Dense (only)
            x = Dropout(rate=0.5, name="ery_dropout")(layer.output)
            output_branches.append(Dense(3, activation='sigmoid', name=str("tiss_ery_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(x))
    # 4.2.2 Excoration
    tmp = []
    for layer in pretrained_model.layers:
        if "tiss_exc_dense512_2" in layer.name: # Add Original 512 Dense
            x = Dropout(rate=0.5, name="exc_dropout")(layer.output)
            tmp.append(x)
    for output in output_branches:
        if "tiss_ery" in output.name: # Add Erythema
            tmp.append(output)
    output_branches.append(Dense(3, activation='sigmoid', name=str("tiss_exc_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(concatenate(tmp)))
    # 4.2.3 Oedema
    tmp = []
    for layer in pretrained_model.layers:
        if "tiss_oed_dense512_2" in layer.name: # Add Original 512 Dense
            x = Dropout(rate=0.5, name="oed_dropout")(layer.output)
            tmp.append(x)
    for output in output_branches:
        if "tiss_ery" in output.name: # Add Erythema
            tmp.append(output)
        if "tiss_exc" in output.name: # Add Excoriation
            tmp.append(output)
    output_branches.append(Dense(3, activation='sigmoid', name=str("tiss_oed_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(concatenate(tmp)))
    # 4.2.4 Lichenification
    tmp = []
    for layer in pretrained_model.layers:
        if "sassad_lic_dense512_2" in layer.name: # Add Original 512 Dense
    ...
    for output in output_branches:
        ...
        if "tiss_exc" in output.name: # Add Excoriation
            tmp.append(output)
    output_branches.append(Dense(3, activation='sigmoid', name=str("sassad_lic_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(concatenate(tmp)))
    # 4.2.5 Exudation
    tmp = []
    for layer in pretrained_model.layers:
        if "sassad_exu_dense512_2" in layer.name: # Add Original 512 Dense
    ...
    for output in output_branches:
        ...
        if "tiss_exc" in output.name: # Add Excoriation
            tmp.append(output)
    output_branches.append(Dense(3, activation='sigmoid', name=str("sassad_exu_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(concatenate(tmp)))
    # 4.2.6 Dryness
    tmp = []
    for layer in pretrained_model.layers:
        if "sassad_dry_dense512_2" in layer.name: # Add Original 512 Dense
    ...
    for output in output_branches:
        ...
        if "tiss_exc" in output.name: # Add Excoriation
            tmp.append(output)
    output_branches.append(Dense(3, activation='sigmoid', name=str("sassad_dry_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(concatenate(tmp)))
    # 4.2.7 Cracking
    tmp = []
    for layer in pretrained_model.layers:
        if "sassad_cra_dense512_2" in layer.name: # Add Original 512 Dense
    ...
    for output in output_branches:
        ...
        if "tiss_exc" in output.name: # Add Excoriation
            tmp.append(output)
        if "sassad_lic" in output.name: # Add Lichenification
            tmp.append(output)
    output_branches.append(Dense(3, activation='sigmoid', name=str("sassad_cra_output"), kernel_constraint=max_norm(3), bias_constraint=max_norm(3))(concatenate(tmp)))
    # -- 4.3 Compile model --
    model = Model(inputs=pretrained_model.input, outputs=output_branches, name="eczemanet")
    # -- 4.4 Freeze pretrained layers --
