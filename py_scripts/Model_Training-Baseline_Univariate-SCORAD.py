...
OUTPUT_PATH = "../output/Baseline_Univariate_SCORAD/"
...
for run in range(0,10):
    ...
    for index, row in meta_train.iterrows():
        ...
        tots.append(tot)
    y_train["scorad_tot_output"].append(label_ordinariser(tots,k_lim=19))
    y_train["scorad_tot_output"] = np.squeeze(np.array(y_train["scorad_tot_output"]))
    ...
    for index, row in meta_test.iterrows():
        ...
        tots.append(tot)
    y_test["scorad_tot_output"].append(label_ordinariser(tots,k_lim=19))
    y_test["scorad_tot_output"] = np.squeeze(np.array(y_test["scorad_tot_output"]))
