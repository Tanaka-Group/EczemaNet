...
OUTPUT_PATH = "../output/Baseline_Whole_Image-Univariate-Regression_SCORAD/"
BRANCHES_LIST = ['scorad_tot']
SCORAD_BRANCHES_LIST = ['tiss_ery', 'tiss_oed', 'sassad_exu', 'tiss_exc', 'sassad_lic', 'sassad_dry']
...
def predict(x_data, meta_data, model, branchlist=BRANCHES_LIST):
    ...
    Y_true["scorad_tot"] =  np.sum( [ Y_true[key] for key in SCORAD_BRANCHES_LIST], axis=0)
...
for run in range(0,10):
    ...
    y_train = {"scorad_tot_output": []}
    y_test = {"scorad_tot_output": []}
    ...
    for index, row in meta_train.iterrows():
        tot = 0
        for branch in SCORAD_BRANCHES_LIST:
            tot += meta_train[branch][index]
        y_train["scorad_tot_output"].append(tot)
    y_train["scorad_tot_output"] = np.array(y_train["scorad_tot_output"])
    ...
    for index, row in meta_test.iterrows():
        ...
        y_test["scorad_tot_output"].append(tot)
    y_test["scorad_tot_output"] = np.array(y_test["scorad_tot_output"])
