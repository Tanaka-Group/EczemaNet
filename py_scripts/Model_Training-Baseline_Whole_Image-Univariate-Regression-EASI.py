...
OUTPUT_PATH = "../output/Baseline_Whole_Image-Univariate-Regression_EASI/"
...
def predict(x_data, meta_data, model, branchlist=BRANCHES_LIST):
    ...
    Y_true["easi_tot"] =  np.sum( [ Y_true[key] for key in EASI_BRANCHES_LIST], axis=0)
...
for run in range(0,10):
    ...
    for index, row in meta_train.iterrows():
        ...
        y_train["easi_tot_output"].append(tot)
    y_train["easi_tot_output"] = np.array(y_train["easi_tot_output"])
    ...
    for index, row in meta_test.iterrows():
        ...
        y_test["easi_tot_output"].append(tot)
    y_test["easi_tot_output"] = np.array(y_test["easi_tot_output"])
