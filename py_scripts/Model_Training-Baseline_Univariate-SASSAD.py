...
OUTPUT_PATH = "../output/Baseline_Univariate_SASSAD/"
...
for run in range(0,10):
    ...
    for index, row in meta_train.iterrows():
        ...
        tots.append(tot)
    y_train["sassad_tot_output"].append(label_ordinariser(tots,k_lim=19))
    y_train["sassad_tot_output"] = np.squeeze(np.array(y_train["sassad_tot_output"]))
    ...
    for index, row in meta_test.iterrows():
        ...
        tots.append(tot)
    y_test["sassad_tot_output"].append(label_ordinariser(tots,k_lim=19))
    y_test["sassad_tot_output"] = np.squeeze(np.array(y_test["sassad_tot_output"]))
