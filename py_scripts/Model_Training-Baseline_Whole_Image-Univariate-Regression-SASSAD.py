...
# # Model Training: Baseline Study - ROI Crops (ROI) VS Whole Image (Univariate+Regression)
...
OUTPUT_PATH = "../output/Baseline_Whole_Image-Univariate-Regression_SASSAD/"
BRANCHES_LIST = ['sassad_tot']
SASSAD_BRANCHES_LIST = ['sassad_cra','sassad_dry','tiss_ery','tiss_exc','sassad_exu','sassad_lic']
...
# Load data:
meta_data = pd.read_csv(os.path.join(PATH_TO_DATASET,"meta_whole_data.csv"))
...
def predict(x_data, meta_data, model, branchlist=BRANCHES_LIST):
    ...
    # 1. Make predictions
    y_crops_pred = model.predict(x_data)
    ...
    for group_name, df_group in meta_grouped:
        ...
        for idx, branchname in enumerate(branchlist):
            y_whole_pred[branchname] = np.mean(y_crops_pred[indexes])

        Y_pred = Y_pred.append(y_whole_pred,ignore_index=True)
        Y_true = Y_true.append(meta_data.loc[indexes[0]],ignore_index=True)

    Y_true["sassad_tot"] =  np.sum( [ Y_true[key] for key in SASSAD_BRANCHES_LIST], axis=0)
...
for run in range(0,10):
    ...
    y_train = {"sassad_tot_output": []}
    y_test = {"sassad_tot_output": []}
    classWeights = {}
    ...
    for index, row in meta_train.iterrows():
        tot = 0
        for branch in SASSAD_BRANCHES_LIST:
            tot += meta_train[branch][index]
        y_train["sassad_tot_output"].append(tot)
    y_train["sassad_tot_output"] = np.array(y_train["sassad_tot_output"])
    ...
    for index, row in meta_test.iterrows():
        ...
        y_test["sassad_tot_output"].append(tot)
    y_test["sassad_tot_output"] = np.array(y_test["sassad_tot_output"])
    ...
    # ------------------------------------------------
    # 1. Make Prediction
    Y_true, Y_pred = predict(x_data=x_test, meta_data=meta_test, model=model, branchlist = BRANCHES_LIST)
    # 2. Evaluate results:
    stat = calculate_statistics(Y_true, Y_pred, branchlist = BRANCHES_LIST)
    stat["run"] = run
